...
const SECRET_PATTERNS: SecretPatternRule[] = [
  ...
  // 20b. Action, Mutation, and Value Update Trigger Phrasings
  {
    ...
    pattern: /\b(?:set\s+(?:the\s+|my\s+|it\s+to\s+|password\s+to\s+|pass\s+to\s+|secret\s+to\s+)|rotate\s+(?:the\s+|it\s+to\s+|password\s+to\s+|secret\s+to\s+)|changed\s+(?:the\s+|it\s+to\s+|password\s+to\s+|secret\s+to\s+)|change\s+(?:password|secret|it)\s+to\s+|reset\s+(?:the\s+|it\s+to\s+|password\s+to\s+|secret\s+to\s+)|updated\s+(?:the\s+|it\s+to\s+|password\s+to\s+|secret\s+to\s+)|update\s+(?:password|secret|it)\s+to\s+|use\s+password\s+|enter\s+password\s+|with\s+password\s+|new\s+value\s+is\s+|value\s+is\s+now\s+|value\s+is\s+|the\s+new\s+value\s+is\s+|current\s+value\s+is\s+|password\s+is\s+now\s+|the\s+password\s+is\s+)\s*[:=]?\s*["']?([a-zA-Z0-9!@#$%^&*()_+\-=\[\]{}|;:,.<>?/~`]{3,128})["']?/gi,
